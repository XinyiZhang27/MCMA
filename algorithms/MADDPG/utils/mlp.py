...
class MLPBase(nn.Module):
    def __init__(self, args, obs_dim):
        ...
        self._use_feature_normalization = args.use_feature_normalization2
        self._use_orthogonal = args.use_orthogonal2
        self._use_ReLU = args.use_ReLU2
        self._use_conv1d = args.use_conv1d
        self._stacked_frames = args.stacked_frames2
        self._layer_N = args.layer_N2
        ...
        if self._use_conv1d:
            ...
            random_x = torch.FloatTensor(1, self._stacked_frames, obs_dim)
            random_out = self.conv(random_x)
            assert len(random_out.shape) == 3
            obs_dim = random_out.size(-1) * random_out.size(-2)
